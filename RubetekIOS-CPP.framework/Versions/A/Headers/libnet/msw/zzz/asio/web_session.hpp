...
namespace zzz {
    ...
    template <>
    struct session<web_socket>
        : noncopyable
        , std::enable_shared_from_this<session<web_socket>>
    {
        typedef web_socket                       socket        ;
...
namespace zzz {
    ...
    inline session<web_socket>::session(socket&& socket, on_received on_received, on_sent on_sent, on_error on_error)
    ...
    inline void session<web_socket>::start()
    {
        do_receive();
    }
    inline void session<web_socket>::cancel()
    {
        stop();
        socket_.close();
    }

    inline void session<web_socket>::send(range<byte const> packet)
    {
    ...
    inline uint session<web_socket>::queue_packets_count() const
    {
        return static_cast<uint>(deque_packets_.size());
    }

    inline size<byte> session<web_socket>::queue_packets_size() const
    {
    ...
    inline session<web_socket>::endpoint_type session<web_socket>::local_endpoint() const
    {
    ...
    inline session<web_socket>::endpoint_type session<web_socket>::remote_endpoint() const
    {
    ...
    template <typename Handler>
    inline void session<web_socket>::eject_socket(Handler handler)
    {
        //socket_.cancel();
        stop();
        auto self(MSW_SHARED_FROM_THIS);
    ...
    inline void session<web_socket>::do_receive()
    {
        auto self(MSW_SHARED_FROM_THIS);
    ...
    inline void session<web_socket>::do_send()
    {
        send_stream_ = std::move(deque_packets_.front());
        send_stream();
    }

    inline void session<web_socket>::send_stream()
    {
        auto self(MSW_SHARED_FROM_THIS);
        socket_.async_send
        (
    ...
    inline void session<web_socket>::stop()
    {
